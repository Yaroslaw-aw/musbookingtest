...
namespace MUSbooking.Common.Caching
{
    public class Redis(IDistributedCache cache)
    {
        ...
        //    cacheEntryOptions.AbsoluteExpirationRelativeToNow = TimeSpan.FromSeconds(TTL);
        ...
        public async Task SetDataAsync<T>(string key, T? value, int TTL)
        {
            ...
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                using (StreamReader reader = new StreamReader(stream))
                {
                    ...
                    DistributedCacheEntryOptions cacheEntryOptions = new DistributedCacheEntryOptions
                    {
                        AbsoluteExpiration = DateTimeOffset.UtcNow + TimeSpan.FromSeconds(TTL)
                    };
        ...
        {
            T? value = await GetDataAsync<T>(key);
